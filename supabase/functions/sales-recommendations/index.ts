...
${retention?.emailCount ? `Email-база: ${retention.emailCount} контактів` : ""}`;
    ...
    const personaIntro = `Ти — AI LeadOслав, особистий AI-помічник маркетолога в навчальному симуляторі ADS School. Відповідай від свого імені — по-людськи, дружньо, наче пишеш колезі, а не як безликий асистент. ВАЖЛИВО: не представляйся, не пиши хто ти, не задавай уточнюючих питань, не пиши вступів типу "Звісно" чи "Ось". Одразу починай з рекомендацій по ділу. Пиши звичайним текстом з емодзі та перенесеннями рядків — БЕЗ markdown-розмітки: не використовуй **, ##, зірочки, решітки, бектики.`;
    const noMdFormat = `Формат: звичайний текст без markdown-розмітки (без **, ##, зірочок, решіток, бектиків), кожен пункт з нового рядка з емодзі.`;

    if (type === "call-script") {
      systemPrompt = `${personaIntro} Ти — досвідчений маркетолог-стратег та експерт з продажів по телефону. Генеруй глибоко опрацьовані скрипти дзвінків українською мовою. Використовуй конкретні формулювання для цієї ніші.`;
...
4. 🛡️ Робота із запереченнями — 7-10 типових заперечень та конкретні відповіді на кожне: "Дорого", "Потрібно подумати", "Я порівнюю з конкурентами", "Не зараз", "Надішліть інформацію на пошту" та інші типові для цієї ніші
5. 🤝 Закриття угоди — 3 техніки закриття з конкретними фразами
6. 📋 Чек-лист після дзвінка — що зробити після розмови

${noMdFormat} Додай конкретні фрази-приклади.`;
    } else if (type === "chat-script") {
      systemPrompt = `${personaIntro} Ти — досвідчений маркетолог-стратег та експерт з продажів у месенджерах. Генеруй глибоко опрацьовані скрипти переписки українською мовою.`;
      userPrompt = `${fullContext}
...
4. 🛡️ Робота із запереченнями в переписці — як відповідати на: "Скільки коштує?" (коли рано говорити ціну), "Чому так дорого?", "Я подумаю", "Чим ви відрізняєтесь?", ігнорування повідомлень — як повернути діалог
...
${noMdFormat} Готові шаблони повідомлень, які можна скопіювати та використовувати.`;
    } else if (type === "follow-up") {
      systemPrompt = `${personaIntro} Ти — досвідчений маркетолог-стратег та експерт з follow-up стратегій. Генеруй розгорнуті рекомендації щодо покращення конверсії та закриття угод українською мовою.`;
      userPrompt = `${fullContext}

Створи детальну follow-up стратегію для цієї ніші. Включи:

1. ⏱️ Таймлайн follow-up — конкретний розклад контактів: через 5 хвилин після ліда, через 1 годину, через 24 години, через 3 дні, через 7 днів, через 14 днів, через 30 днів
...
${noMdFormat} Розгорнута стратегія з конкретними прикладами та шаблонами.`;
    } else if (type === "conclusion") {
      systemPrompt = `${personaIntro} Ти — досвідчений маркетолог-стратег. Зроби загальний висновок та стратегічне резюме по всій маркетинговій воронці українською мовою. Будь конкретним, давай цифри та чіткі рекомендації.`;
      userPrompt = `${fullContext}
