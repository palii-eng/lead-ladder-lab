...
const FlowNode: React.FC<FlowNodeProps> = ({ icon, title, index, isActive, isCompleted, isLast, isLocked, subtitle, onClick }) => {
  return (
    <div className="flex items-start flex-shrink-0">
      {/* Node column — centered */}
      <div className="flex flex-col items-center" style={{ minWidth: isActive ? '84px' : '64px' }}>
        <button
          onClick={onClick}
          disabled={isLocked}
        ...
        <button
          onClick={onClick}
          disabled={isLocked}
          className={`mt-2 text-xs font-semibold max-w-[90px] text-center leading-tight transition-colors ${
        ...
        {subtitle && (
          <div className="flex flex-col items-center gap-0.5 mt-1">
            {subtitle.split('\n').map((line, idx) => (
              <span key={idx} className="text-[9px] text-primary font-medium max-w-[200px] text-center leading-tight whitespace-nowrap bg-primary/10 px-2 py-0.5 rounded-full">
                {line}
              </span>
            ))}
          </div>
      ...
      {/* Connector — separate from node, vertically centered to circle */}
      {!isLast && (
        <div className="flex items-center" style={{ height: isActive ? '84px' : '64px' }}>
          <svg width="80" height="16" viewBox="0 0 80 16" className="flex-shrink-0 mx-2">
            <line
